...
export default function ProductionPOModule({ token, userRole }) {
  ...
  const [form, setForm] = useState({ po_number: '', customer_name: '', vendor_id: '', po_date: '', deadline: '', delivery_deadline: '', notes: '', items: [] });
  const [editForm, setEditForm] = useState({});

  const isSuperAdmin = userRole === 'superadmin';
  const canEdit = ['superadmin', 'admin'].includes(userRole);

  useEffect(() => { fetchPOs(); fetchProducts(); fetchVendors(); }, []);
  ...
  const openCreate = () => {
    setEditData(null);
    setForm({ po_number: '', customer_name: '', vendor_id: '', po_date: new Date().toISOString().split('T')[0], deadline: '', delivery_deadline: '', notes: '', items: [] });
    setShowModal(true);
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input required className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.customer_name} onChange={e => setForm({...form, customer_name: e.target.value})} placeholder="PT. Nama Customer" />
              </div>
            </div>
            <div>
