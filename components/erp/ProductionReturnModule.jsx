          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button type="button" onClick={addItem} className="flex items-center gap-1 text-xs text-red-600 hover:text-red-800 font-medium">
                  <Plus className="w-3.5 h-3.5" /> Tambah Item
                </button>
              </div>
              {form.items.length === 0 && (
                <p className="text-sm text-slate-400 italic text-center py-4 border border-dashed border-slate-200 rounded-lg">
                  Klik "Tambah Item" untuk menambahkan produk yang diretur
                </p>
              )}
                ...
                {form.items.map((item, idx) => (
                  <div key={idx} className="border border-red-100 rounded-xl p-3 bg-red-50/30">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        <label className="block text-xs text-slate-600 mb-1">Nama Produk *</label>
                        <input required className="w-full border border-slate-200 rounded px-2 py-1.5 text-xs" value={item.product_name} onChange={e => updateItem(idx, 'product_name', e.target.value)} placeholder="Nama Produk" />
                      </div>
                      <div>
                      ...
                      <div>
                        <label className="block text-xs text-slate-600 mb-1">Qty Retur *</label>
                        <input required type="number" min="1" className="w-full border border-slate-200 rounded px-2 py-1.5 text-xs" value={item.return_qty} onChange={e => updateItem(idx, 'return_qty', e.target.value)} placeholder="10" />
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-2 mt-2">
                      <div>
                        <label className="block text-xs text-slate-600 mb-1">No. Seri / Batch</label>
                        <input className="w-full border border-amber-200 rounded px-2 py-1.5 text-xs font-mono bg-amber-50 focus:outline-none focus:ring-1 focus:ring-amber-400" value={item.serial_number || ''} onChange={e => updateItem(idx, 'serial_number', e.target.value)} placeholder="SN-2025-001" />
                      </div>
                      <div>
                    ...
                    <div className="mt-2">
                      ...
                      <input className="w-full border border-slate-200 rounded px-2 py-1.5 text-xs" value={item.repair_notes} onChange={e => updateItem(idx, 'repair_notes', e.target.value)} placeholder="Deskripsi perbaikan yang diperlukan..." />
                    </div>
                  </div>
                ))}
              </div>
